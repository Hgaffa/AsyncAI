...
timeout, and idempotent restart (GTHR-01 through GTHR-06).
"""
...
async def gather(
    ...
    *,
    ...
    """
    # Step 1: Guard — must be called inside a @workflow function
    ...
    # Step 2: Ensure the Workflow row exists (required by FK constraint on job.workflow_id).
    # When gather() is called directly in tests (bypassing the @workflow runner), the row
    # may not exist yet. Check first to avoid duplicate insert.
    ...
    # Step 3: Idempotency check — look for existing child jobs for this step (GTHR-06)
    async with AsyncSessionFactory() as session:
    ...
    if existing_jobs:
        ...
        # Step 4: Submit each coroutine with workflow context already set in ContextVars.
    ...
    # Step 5: Spawn an inline worker to process submitted jobs.
    ...
    # Step 6: Poll loop — wait for all jobs to reach a terminal state
    ...
    try:
        while True:
            ...
            # Check for any FAILED job (GTHR-04)
            for job in jobs:
            ...
            # Check if all jobs completed (GTHR-02)
    ...
    # Step 7: Fetch results in submission order (GTHR-03)
    results: list[Any] = []
    async with AsyncSessionFactory() as session:
